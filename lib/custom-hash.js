...
function multiply (x, y, base) {
    ...
    return x.reduce(function (sum, currentX, indexX) {
        offset = Array.apply(null, new Array(indexX)).map(function () { return 0; });

        summand = y.reduce(function (previous, currentY, indexY) {
            value = currentX * currentY + previous.carry;
            digit = value % base;
            carry = (value - digit) / base;

            if ((indexY === y.length - 1) && (carry > 0)) {
